...
## FLUJO COMPLETO DEL MODELO DE DIFUSIÓN (OVERVIEW)

### Paso 1: Tienes un dataset de imágenes
```
[imagen1, imagen2, imagen3, ..., imagen_n]
```

### Paso 2: ENTRENAMIENTO
```
Repetir muchas veces:
    1. Tomar una imagen del dataset (x₀)
    2. Elegir un paso aleatorio (ej: t=500)
    3. Generar ruido aleatorio (ε)
    4. Crear versión ruidosa: x₅₀₀ = √ᾱ₅₀₀ × x₀ + √(1-ᾱ₅₀₀) × ε
       (esto es el "atajo" - saltas directo al paso t en 1 operación)
    5. Darle x₅₀₀ y t=500 al modelo
    6. El modelo predice qué ruido hay (ε_predicho)
    7. Comparar ε vs ε_predicho (función de pérdida)
    8. Ajustar pesos del modelo
```
**Resultado:** Un modelo que sabe predecir cuánto ruido hay en una imagen.

### Paso 3: GENERACIÓN (crear imagen nueva)
```
1. Crear ruido puro aleatorio (x₁₀₀₀)
2. Para t = 1000, 999, 998, ..., 1:
    - Darle xₜ y t al modelo
    - Modelo predice el ruido
    - Restar el ruido: xₜ₋₁ = xₜ - ruido_predicho
3. x₀ es la imagen generada
```
**Resultado:** Una imagen nueva que no existía en el dataset.

### Diagrama del flujo:
```
ENTRENAMIENTO (usa atajo para crear datos rápido):
    Dataset → Elegir imagen → Elegir t → Crear xₜ directo → Modelo aprende

GENERACIÓN (paso a paso obligatorio):
    Ruido puro → Modelo quita ruido → ... (1000 veces) → Imagen nueva
```

### ¿Por qué existe el atajo?

El atajo solo sirve para el **entrenamiento**. Sin él, para crear una imagen con ruido del paso 500, tendrías que calcular los 499 pasos anteriores (500 operaciones). Con el atajo, es 1 operación.

La **generación** NO tiene atajo. El modelo necesita ver cada estado intermedio para decidir qué ruido quitar.
