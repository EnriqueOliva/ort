...
20. Run 15. ChannelShuffle added, color aug p=0.50, ToGray p=0.30, dropout=0.25, eta_min=1e-4.
...
### ✅ Run 14: CosineAnnealingLR Restored, Cleaner Augmentation

**Status:** ✅ COMPLETED - 700 epochs

**Changes from Run 13:**
- Reverted to CosineAnnealingLR (removed custom LambdaLR asymmetric schedule)
- eta_min reduced to 5e-7 (twice as low as before)
- Removed CoarseDropout from augmentation
- Removed GaussNoise from augmentation
...
| **Median Dice** | 0.9809 | **0.9819** | +0.10% |
| **Std Dev** | 0.0723 | **0.0807** | +11.6% (slightly worse) |
| **Min Dice (worst)** | 0.2897 | **0.3533** | **+22.0%** ✅ |
| **Max Dice** | 0.9970 | **0.9982** | +0.12% |
| **Threshold** | 0.45 | **0.50** | Changed |

**Training Details:**
- Epochs completed: 700
- Training duration: 19h 53m
- Best Val Dice: ~0.9558

**Key Observations:**
1. ✅ Mean Dice slightly improved (+0.08%)
2. ✅ **Worst case improved significantly: 0.2897 → 0.3533 (+22%)**
3. ⚠️ Standard deviation increased slightly (more variance)
4. Model is more confident (threshold 0.45 → 0.50)

**Worst Case Analysis:**

From visual inspection of worst predictions:
- Model still confuses **skin-colored backgrounds** (pink blankets, orange rocks, wooden textures)
- Model struggles with **B&W images** without color cues
- Model **under-segments white/light clothing** (bride's dress)
- Model segments **objects near humans** (bikes, backpacks)

**Pattern:** Model learned "skin-colored/textured pixels" not "human shape". This is a fundamental limitation of vanilla U-Net without attention mechanisms or pretrained encoders.

**Why Run 14 Improved Worst Case:**
- Removing CoarseDropout: less aggressive regularization
- Removing GaussNoise: cleaner training signal
- Lower eta_min: more fine-tuning in late epochs
- 700 epochs: more time to converge on difficult cases

---

### 🔄 Run 15: Color Invariance + Stronger Regularization (PLANNED)

**Status:** 🔄 PLANNED

**Hypothesis:** Run 14 failures are dominated by color confusion (skin-colored backgrounds). Adding ChannelShuffle and stronger color augmentation should force the model to learn shape, not color.
...
| **Color aug probability** | p=0.35 | **p=0.50** | More color variation exposure |
| **ToGray** | p=0.20 | **p=0.30** | Better B&W image handling |
| **Dropout** | p=0.10 | **p=0.25** | Compensate for more augmentation, reduce overfitting |
| **eta_min** | 5e-7 | **1e-4** | Keep learning productive in late epochs |

**Expected Impact:**

| Change | Target Problem |
|--------|----------------|
| ChannelShuffle | Pink blankets, orange rocks, wood segmented as skin |
| ↑ Color aug p=0.50 | Overall color confusion |
| ↑ ToGray p=0.30 | B&W image failures |
| ↑ Dropout p=0.25 | Overfitting, train-val gap |
| ↑ eta_min | Wasted late epochs with negligible LR |

**Data Augmentation (Run 15):**
```python
A.OneOf([
    A.HueSaturationValue(...),
    A.ColorJitter(...),
    A.ChannelShuffle(p=1.0),  # NEW
], p=0.50),  # CHANGED from 0.35

A.ToGray(p=0.30),  # CHANGED from 0.20
```

**Realistic Expectations:**
- Mean Dice: 0.96-0.97 (slight improvement)
- Worst Case: 0.40-0.50 (target: break color confusion pattern)
