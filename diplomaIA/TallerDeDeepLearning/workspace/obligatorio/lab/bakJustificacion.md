...
- Se usa Albumentations para transformaciones y data augmentation. Albumentations tiene una funcionalidad útil y es que aplica la misma transformación a imagen y máscara automáticamente.
    ...
    - HorizontalFlip activado. Una persona mirando a la izquierda es igual de válida que una mirando a la derecha.
    ...
    - GaussNoise con probabilidad baja (3%). El survey menciona que "random noise is one of the simplest intensity transformation algorithms". Se usa con baja probabilidad porque el dataset son fotos de buena calidad.
    - Normalización con valores estándar de ImageNet. Práctica común que ayuda a la convergencia.
...
- Clase SegmentationDataset custom que carga imagen y máscara, aplica transformaciones, y las devuelve como tensores. Soporta modo test (sin máscara).
...
- Mixed Precision Training (AMP) con autocast y GradScaler. Reduce uso de memoria ~50% y acelera el entrenamiento. Esto no se vio en clase, se investigó por cuenta apropia para poder entrenar con mayor resolución.
...
- Early stopping implementado pero desactivado en la versión final. Con CosineAnnealingLR el modelo sigue mejorando hasta el final.

### Entrenamiento del modelo

- Respecto a la duración del entrenamiento:
  - En esta versión final se entrenó durante 600 épocas.
  - Se entrenó múltiples veces utilizando distintos valores, 20 entrenamientos registrados en Wandb (algunos entrenamientos adicionales se descartaron por constatar errores en la configuración)
...
- El modelo supera ampliamente el requisito mínimo de Dice >= 0.75 establecido en el obligatorio, alcanzando un Dice medio de 0.9562 en validación.
- La mediana (0.9804) es significativamente mayor que la media (0.9562), lo que indica que la mayoría de las predicciones son muy buenas pero hay algunos outliers que bajan el promedio.
- El Dice máximo de 0.9972 muestra que el modelo puede lograr predicciones casi perfectas cuando la imagen no presenta ambigüedades.
- El Dice mínimo de 0.1222 corresponde a los casos problemáticos identificados en la sección de evaluación (fondos con colores similares a piel).
- La diferencia entre Train Dice (0.9857) y Val Dice (0.9543) es pequeña, indicando que no hay overfitting severo.
