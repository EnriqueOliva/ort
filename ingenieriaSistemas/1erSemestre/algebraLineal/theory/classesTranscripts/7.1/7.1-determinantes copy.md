...
> **Ejercicio — Propiedad 1 (hecho en clase)**
>
> Una matriz $4 \times 4$ tiene la columna 4 expresada como: columna 4 = columna 1 + columna 2. ¿Cuánto vale su determinante?
>
> **Paso 1:** La columna 4 es una suma de dos cosas (columna 1 y columna 2). Por propiedad 1, separamos en dos determinantes.
>
> **Paso 2:** En el primer determinante, la columna 4 quedó igual a la columna 1 → dos columnas iguales → $\det = 0$ (propiedad 5).
>
> **Paso 3:** En el segundo determinante, la columna 4 quedó igual a la columna 2 → dos columnas iguales → $\det = 0$ (propiedad 5).
>
> **Resultado:** $0 + 0 = 0$.
...
> **Ejercicio — Propiedad 2**
>
> Calcular $\det\begin{pmatrix} 6 & 12 \\ 3 & 5 \end{pmatrix}$ sacando factor común.
>
> **Paso 1:** Miramos la fila 1: tiene $6$ y $12$. Ambos son múltiplos de $6$. Entonces $6$ es el factor común.
>
> **Paso 2:** Sacamos el $6$ afuera de la fila 1 (propiedad 2): $\det\begin{pmatrix} 6 & 12 \\ 3 & 5 \end{pmatrix} = 6 \cdot \det\begin{pmatrix} 1 & 2 \\ 3 & 5 \end{pmatrix}$
>
> **Paso 3:** Calculamos el determinante de la $2 \times 2$ simplificada: $5 - 6 = -1$
>
> **Resultado:** $6 \cdot (-1) = -6$
...
**Error común en parciales:** Pensar que $\det(4A) = 4 \cdot \det(A)$. **NO.** El $4$ sale una vez por cada fila, no una sola vez.

---

> **Ejercicio — Propiedad 3**
>
> $A$ es $3 \times 3$ y $\det(A) = 5$. ¿Cuánto vale $\det(4A)$?
>
> **Paso 1:** $4A$ significa que TODAS las entradas están multiplicadas por $4$. Eso es lo mismo que decir: las 3 filas están multiplicadas por $4$.
>
> **Paso 2:** Sacamos el $4$ de la fila 1 (propiedad 2): queda $4 \cdot \det(\text{resto})$
>
> **Paso 3:** Sacamos el $4$ de la fila 2 (propiedad 2 de nuevo): queda $4 \cdot 4 \cdot \det(\text{resto})$
>
> **Paso 4:** Sacamos el $4$ de la fila 3 (propiedad 2 una vez más): queda $4 \cdot 4 \cdot 4 \cdot \det(A) = 4^3 \cdot 5$
>
> **Resultado:** $64 \cdot 5 = 320$. No es $4 \cdot 5 = 20$.
...
> **Ejercicio — Propiedad 4**
>
> $A$ es $2 \times 2$ y $\det(A) = 2$. Calcular $\det(A^3)$.
>
> **Paso 1:** $A^3$ es un producto: $A \cdot A \cdot A$.
>
> **Paso 2:** Por propiedad 4, el determinante de un producto es el producto de los determinantes: $\det(A^3) = \det(A \cdot A \cdot A) = \det(A) \cdot \det(A) \cdot \det(A)$
>
> **Paso 3:** Eso es $\det(A)^3 = 2^3 = 8$.
>
> **Resultado:** $\det(A^3) = 8$
...
> **Ejercicio — Propiedad 5 (ejercicio 7.3 de clase)**
>
> ¿Puede una matriz con dos filas iguales ser invertible?
>
> **Paso 1:** Si tiene dos filas iguales, por propiedad 5 su determinante es $0$.
>
> **Paso 2:** Si el determinante es $0$, por el Teorema 1 la matriz no es invertible.
>
> **Resultado:** No, nunca. Dos filas iguales → det = 0 → no invertible.
>
> **Ejemplo:** $\det\begin{pmatrix} 1 & 2 & 3 \\ 4 & 5 & 6 \\ 1 & 2 & 3 \end{pmatrix} = 0$ porque fila 1 = fila 3 = $(1, 2, 3)$.
...
> **Ejercicio — Propiedad 6**
>
> Calcular $\det(A)$ sabiendo que $\det(B) = 10$, y que $A$ se obtiene intercambiando las filas 1 y 3 de $B$:
>
> $$B = \begin{pmatrix} 3 & 2 & 4 \\ 4 & 6 & 8 \\ 1 & 1 & 6 \end{pmatrix} \implies A = \begin{pmatrix} 1 & 1 & 6 \\ 4 & 6 & 8 \\ 3 & 2 & 4 \end{pmatrix}$$
>
> **Paso 1:** $A$ se obtuvo de $B$ intercambiando fila 1 con fila 3. Un intercambio de filas.
>
> **Paso 2:** Por propiedad 6, un intercambio cambia el signo: $\det(A) = -\det(B)$.
>
> **Resultado:** $\det(A) = -10$.
...
> **Ejercicio — Propiedad 7**
>
> Calcular $\det\begin{pmatrix} 1 & 2 & 3 \\ 4 & 5 & 6 \\ 7 & 8 & 9 \end{pmatrix}$ generando ceros con la propiedad 7 y luego desarrollando por cofactores.
>
> **Paso 1 — Generar ceros en la columna 1.** Le restamos múltiplos de la fila 1 a las otras filas para que la columna 1 tenga ceros:
>
...
>
> La matriz queda: $B = \begin{pmatrix} 1 & 2 & 3 \\ 0 & -3 & -6 \\ 0 & -6 & -12 \end{pmatrix}$. Por propiedad 7, $\det(A) = \det(B)$.
>
> **Paso 2 — Desarrollar por la columna 1** (tiene dos ceros, así que solo el primer elemento aporta):
>
> $\det(B) = 1 \cdot (+1) \cdot \det\begin{pmatrix} -3 & -6 \\ -6 & -12 \end{pmatrix}$
>
> **Paso 3 — Calcular la $2 \times 2$:** $(-3)(-12) - (-6)(-6) = 36 - 36 = 0$
>
> **Resultado:** $\det(A) = 0$
...
> **Ejercicio — Propiedad 8 (ejercicio 5 de clase)**
>
> Si $A$ es ortogonal (es decir, $A^{-1} = A^T$), probar que $\det(A) = 1$ o $\det(A) = -1$.
>
> **Paso 1:** Si $A^{-1} = A^T$, entonces $A \cdot A^T = I$ (definición de inversa).
>
> **Paso 2:** Tomamos determinante de ambos lados: $\det(A \cdot A^T) = \det(I) = 1$.
>
> **Paso 3:** Por propiedad 4: $\det(A) \cdot \det(A^T) = 1$.
>
> **Paso 4:** Por **propiedad 8**: $\det(A^T) = \det(A)$. Reemplazamos: $\det(A) \cdot \det(A) = 1$.
>
> **Paso 5:** Eso es $\det(A)^2 = 1$. Las soluciones son $\det(A) = 1$ o $\det(A) = -1$.
...
2. **Combinada con la propiedad 7:** Si la matriz no es triangular, podés usar la propiedad 7 (sumar filas para generar ceros) hasta convertirla en triangular, y después multiplicar la diagonal.

---

> **Ejercicio — Propiedad 9 (ejercicio 6 de clase)**
>
> ¿Es invertible $A = \begin{pmatrix} 7 & 3 & 5 \\ 0 & 6 & 2 \\ 0 & 0 & 1 \end{pmatrix}$?
>
> **Paso 1:** La matriz es triangular superior (todos ceros debajo de la diagonal).
>
> **Paso 2:** Por propiedad 9, $\det(A) = 7 \cdot 6 \cdot 1 = 42$.
>
> **Paso 3:** $42 \neq 0$, entonces por el Teorema 2, $A$ es invertible.
>
> **Resultado:** Sí. Toda triangular con diagonal no nula es invertible (P9 + Teorema 2).

> Sí. Por propiedad 9, $\det = $ producto de la diagonal. Si todas las entradas de la diagonal son $\neq 0$, el producto es $\neq 0$. Si $\det \neq 0$, la matriz es invertible (Teorema 2).
