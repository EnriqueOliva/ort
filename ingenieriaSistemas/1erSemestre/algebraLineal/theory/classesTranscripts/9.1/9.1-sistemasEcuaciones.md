...
Queremos generar un $0$ en la posición $(2,1)$. Aplicamos: $F_2 \leftarrow -2 \cdot F_2 + F_1$

- Nueva fila 2 columna 1: $-2 \cdot 1 + 2 = 0$ ✓
- Nueva fila 2 columna 2: $-2 \cdot 1 + (-1) = -3$
- Nueva fila 2 término indep.: $-2 \cdot 3 + 0 = -6$

$$\left(\begin{array}{cc|c} 2 & -1 & 0 \\ 0 & -3 & -6 \end{array}\right)$$

Si además dividimos la fila 2 por $-3$ (o sea, $F_2 \leftarrow -\tfrac{1}{3} F_2$):

$$\left(\begin{array}{cc|c} 2 & -1 & 0 \\ 0 & 1 & 2 \end{array}\right)$$

### Paso 3 — Clasificar

Escalera perfecta, 2 ecuaciones efectivas, 2 incógnitas → **sistema compatible determinado**.

### Paso 4 — Resolver

De la fila 2: $y = 2$.

De la fila 1: $2x - y = 0 \Rightarrow 2x - 2 = 0 \Rightarrow x = 1$.

**Solución:** $x = 1, \; y = 2$.

### Interpretación geométrica

Cada ecuación lineal de 2 variables es una **recta** en el plano. El sistema pregunta por los puntos que están en **las dos rectas** al mismo tiempo.

- Recta 1: $2x - y = 0$ ⇒ $y = 2x$ (pasa por el origen, pendiente $2$).
- Recta 2: $x + y = 3$ (pasa por $(3,0)$ y $(0,3)$).

Las dos rectas se cortan en un solo punto: $(1, 2)$. Eso es exactamente la solución.

> "Cuando las rectas se cortan, el sistema es compatible determinado y el punto de corte es la solución"

**Regla geométrica 2×2:**

| Cómo se ven las rectas | Clasificación |
|------------------------|---------------|
| Se cortan en un solo punto | SCD |
| Son paralelas (nunca se cortan) | SI |
| Son coincidentes (la misma recta) | SCI (infinitos puntos en común) |
...
$$\left(\begin{array}{cc|c} 1 & -1 & 1 \\ -2 & 2 & 4 \end{array}\right) \xrightarrow{F_2 \leftarrow 2F_1 + F_2} \left(\begin{array}{cc|c} 1 & -1 & 1 \\ 0 & 0 & 6 \end{array}\right)$$

### Clasificar

La fila 2 quedó "$0 = 6$", que es **imposible**. Poniendo el $6$ arriba del $2$ si lo normalizamos, igual es una fila de ceros con un número $\neq 0$ a la derecha → **sistema incompatible**.

### Geométricamente

- Recta 1: $x - y = 1$
- Recta 2: $-2x + 2y = 4$ ⇒ dividiendo por $-2$: $x - y = -2$

Son dos rectas paralelas (misma pendiente, distinto término independiente). Nunca se cortan → no hay solución → incompatible.
...
$$\left(\begin{array}{cc|c} 2 & -1 & 1 \\ -4 & 2 & -2 \end{array}\right) \xrightarrow{F_2 \leftarrow 2F_1 + F_2} \left(\begin{array}{cc|c} 2 & -1 & 1 \\ 0 & 0 & 0 \end{array}\right)$$

### Clasificar

La fila 2 quedó "$0 = 0$", que siempre se cumple. Perdimos una ecuación. Queda una sola ecuación efectiva ($p = 1$) con dos incógnitas ($n = 2$), así que $r = n - p = 1$ grado de libertad → **sistema compatible indeterminado**.

### Resolver

La ecuación efectiva es $2x - y = 1$.

Tomamos un parámetro para el grado de libertad. Por ejemplo, $x = \alpha$:

$$y = 2x - 1 = 2\alpha - 1$$

**Solución general:** $\quad x = \alpha, \quad y = 2\alpha - 1, \quad \alpha \in \mathbb{R}$.

### Otra forma de expresarlo

> "Son dos formas distintas de expresarlo. Acá, como tenemos dos incógnitas nomás, $x$ e $y$, capaz que tiene más sentido esta forma. Cuando empezamos a tener más incógnitas para decir, bueno, la $y$ vale $\alpha$, ya empieza a despejar y no marearse, me parece que empieza a tomar más sentido"

También se puede escribir:

$$\text{Solución} = \{(x, y) \in \mathbb{R}^2 : y = 2x - 1\}$$

Las dos formas describen lo mismo. Con parámetro es más cómodo cuando hay muchas incógnitas.

### Geométricamente

Si dividís la segunda ecuación por $-2$, obtenés $2x - y = 1$ — **la misma recta que la primera**. Las dos rectas son coincidentes (son exactamente la misma recta). Todos los puntos de esa recta son solución → infinitas soluciones.
...
(Nota: los números exactos de la clase no eran perfectamente legibles en la transcripción — se reconstruye el patrón con números que ilustran el mismo fenómeno.)

### Matriz ampliada

$$\left(\begin{array}{ccc|c} 1 & 2 & 1 & 3 \\ 3 & 6 & 1 & 9 \\ 4 & 8 & 2 & 12 \\ 1 & 2 & 1 & 3 \end{array}\right)$$

### Escalerizar generando ceros en la columna 1

Aplicamos tres operaciones al mismo tiempo:
...
La fila 4 se anuló de entrada (era igual a la 1). Las filas 2 y 3 quedaron iguales. Aplicando $F_3 \leftarrow F_3 - F_2$ queda otra fila de ceros:

$$\left(\begin{array}{ccc|c} 1 & 2 & 1 & 3 \\ 0 & 0 & -2 & 0 \\ 0 & 0 & 0 & 0 \\ 0 & 0 & 0 & 0 \end{array}\right)$$

### Clasificar

Dos filas efectivas ($p = 2$), tres incógnitas ($n = 3$) → $r = n - p = 1$ grado de libertad → **sistema compatible indeterminado**.
