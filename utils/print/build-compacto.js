...
const PRINT_CSS = join(__dirname, 'print-compacto.css');
...
async function convert(inputPath) {
  ...
  const outputPath = fullPath.replace(/\.md$/i, '-compacto.pdf');
  ...
  const result = await mdToPdf(
    { content: processed },
    {
      ...
      pdf_options: {
        format: 'A4',
        margin: { top: '11mm', right: '10mm', bottom: '12mm', left: '10mm' },
        printBackground: true,
        displayHeaderFooter: true,
        headerTemplate: '<div></div>',
        footerTemplate: '<div style="font-size:7pt;text-align:center;width:100%;color:#666;"><span class="pageNumber"></span>/<span class="totalPages"></span></div>',
      },
    }
  );
...
if (files.length === 0) {
  console.error('Uso: node print/build-compacto.js <archivo.md> [archivo2.md ...]');
  process.exit(1);
}
...
for (const file of files) {
  ...
  process.stdout.write(`Generando ${name}-compacto.pdf... `);
  try {
